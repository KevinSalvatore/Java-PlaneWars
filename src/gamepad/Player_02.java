...
public class Player_02 extends Plane {
    private boolean enable = true;

    Player_02(int pos_x, int pos_y, String path, int angel) {
    ...
    public void update() {
        ...
        if (inputMsg.getKey(KeyEvent.VK_ENTER)) {
            if (enable) {
        ...
        if (inputMsg.getKey(KeyEvent.VK_UP)) {
            if (inputMsg.getKey(KeyEvent.VK_LEFT)) {
                this.pos_x -= 2;
                this.pos_y -= 2;
                angel = 315;
                image = GameUtil.rotateImage(IMAGE, angel);
            } else if (inputMsg.getKey(KeyEvent.VK_RIGHT)) {
                this.pos_x += 2;
                this.pos_y -= 2;
                angel = 45;
                image = GameUtil.rotateImage(IMAGE, angel);
        ...
        if (inputMsg.getKey(KeyEvent.VK_DOWN)) {
            if (inputMsg.getKey(KeyEvent.VK_LEFT)) {
                this.pos_x -= 2;
                this.pos_y += 2;
                angel = 225;
        ...
        if (inputMsg.getKey(KeyEvent.VK_RIGHT)) {
            this.pos_x += 5;
            angel = 90;
            image = GameUtil.rotateImage(IMAGE, angel);
        }
    ...
    public void draw(Graphics g) {
        super.draw(g);
        g.drawString("Julia".toUpperCase(), 13 * Conf.WIDTH / 14 - 40, 40);
        for (int i = 12; i > 12 - life; i--) {
